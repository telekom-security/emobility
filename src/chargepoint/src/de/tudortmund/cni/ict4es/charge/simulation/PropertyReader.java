...
public class PropertyReader {
	...
	public PropertyReader(String propertyFileName) throws Exception {
		...
		try {
			BufferedInputStream stream = new BufferedInputStream(
					new FileInputStream(this.propertyFileName));
			prop = new Properties();
			prop.load(stream);
			stream.close();
		} catch (Exception e) {
	...
	public String getPropertyValue(String key) throws Exception {
		...
		if (!prop.containsKey(key)) {
			throw new Exception("Key " + key
					+ " is not present in config file: " + propertyFileName);
		}
		String value = prop.getProperty(key);
		if (isEmpty(value)) {
			throw new Exception("Key " + key + " is present in config file: "
					+ propertyFileName + " but has no value.");
		}
		return value;
	}
	...
	public int getPropertyValueInt(String key) throws Exception {
		...
		try {
			...
			throw new Exception("Value for Key : " + key
					+ " must be a number but is: " + value);
		}
		return valueInt;
	}
	...
	public boolean getPropertyValueBool(String key) throws Exception {
		...
		try {
			...
			throw new Exception("Value for Key : " + key
					+ " must be a boolean value (true,false) but is: " + value);
		}
		return valueBoolean;
	}

	public List<String> getPropertyListValue(String key, String separator)
			throws Exception {
		if (key == null) {
			throw new Exception("Key must be given!");
		}
	...
	public String toString() {
		return "PropertyReader [propertyFileName=" + propertyFileName
				+ ", prop=" + prop + "]";
	}
